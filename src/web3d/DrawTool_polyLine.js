...
class DrawTools {
  ...
  drawPolyGon(lonAndLat) {
    ...
    this.viewer.entities.add({
      name: 'polyGon_name',
      polyline: {
        positions: Cesium.Cartesian3.fromDegreesArray(lonLats(lonAndLat)),
        width: 6,
        clampToGround: true,
  ...
  handleLeftClick(event) {
    ...
    if (pick && pick.id && pick.id.polyline && !isDrawLine) {
  ...
  createEditPoints(pick) {
    ...
    if (pick.id.polyline.positions._callback) {
      // 获取callbackproperty的数据
      positions = pick.id.polyline.positions._callback();
    }else{
      positions = pick.id.polyline.positions._value;
    }
  ...
  handleMouseMoveEditing(event) {
    ...
    // 实时更新多边形的坐标
    pointAndLineEntity.selectedPolygon[0].id.polyline.positions = new Cesium.CallbackProperty(() => {
      return positions;
    }, false);
